...
from fs_mol.utils.distance_utils import compute_similarities, similar_dissimilar_indices
...
class SimilarityTaskSampler(TaskSampler):
    def __init__(
        self,
        train_size_or_ratio: Union[int, float] = 128,
        valid_size_or_ratio: Union[int, float] = 0.0,
    ...
    def sample(self, task: FSMolTask, seed: int = 0) -> FSMolTaskSample:
        ...
        similarity_matrix = compute_similarities(train_samples, test_samples)
        similar_indices, dissimilar_indices = similar_dissimilar_indices(similarity_matrix, 0.5)

        test_samples = [test_samples[i] for i in dissimilar_indices]
