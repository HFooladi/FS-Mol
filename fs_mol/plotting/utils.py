...
#from pandas.core.base import DataError (pandas < 1.5.0)
from pandas.errors import DataError #(pandas >= 1.5.0)
...
def plot_by_size(
    ...
    categories = {x: i for i, x in enumerate(vals["kNN"].index)}
    if highlight_class is not None:
        assert (
...
def plot_by_size_new(
    ...
    markers = ["s", "P", "*", "X", "^", "o", "D", "p"]
    color_set = ["red", "red", "red", "blue", "blue", "blue", "black"]
    linestyles = ["-", "--", ":", "-", "--", ":", "-"]
