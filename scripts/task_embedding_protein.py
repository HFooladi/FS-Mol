...
def get_protein_embedding(esm2_model: str = "esm2_t36_3B_UR50D", layer: int = 36) -> Tuple:
    ...
    target_train_df = pd.read_csv(
        os.path.join(FS_MOL_DATASET_PATH, "targets", "train_proteins.csv")
    )
    target_valid_df = pd.read_csv(
        os.path.join(FS_MOL_DATASET_PATH, "targets", "valid_proteins.csv")
    )
...
def main():
    ...
    protein_hardness_df.to_csv(
        os.path.join(FS_MOL_DATASET_PATH, "targets", "esm2_output", args.esm2_model, f"protein_hardness_{args.k_nearest}.csv"), index=False
    )
