...
def get_protein_embedding(esm2_model : str = "esm2_t36_3B_UR50D", layer : int = 36) -> Tuple:
    ...
    for chembl_id in tqdm(target_train_df['chembl_id']):
        target_accession_id = target_train_df['target_accession_id'][target_train_df['chembl_id'] == chembl_id].item()
        id = train_ids.index(target_accession_id)
        train_emb_tensor = torch.cat((train_emb_tensor, train_emb[id]['mean_representations'][layer][None, :]), 0)
        train_accession_ids.append(target_accession_id)


    for chembl_id in tqdm(target_valid_df['chembl_id']):
        target_accession_id = target_valid_df['target_accession_id'][target_valid_df['chembl_id'] == chembl_id].item()
        id =valid_ids.index(target_accession_id)
        valid_emb_tensor = torch.cat((valid_emb_tensor, valid_emb[id]['mean_representations'][layer][None, :]), 0)
        valid_accession_ids.append(target_accession_id)


    for chembl_id in tqdm(target_test_df['chembl_id']):
        target_accession_id = target_test_df['target_accession_id'][target_test_df['chembl_id'] == chembl_id].item()
        id = test_ids.index(target_accession_id)
        test_emb_tensor = torch.cat((test_emb_tensor, test_emb[id]['mean_representations'][layer][None, :]), 0)
        test_accession_ids.append(target_accession_id)

    
    return train_emb_tensor, valid_emb_tensor, test_emb_tensor, train_accession_ids, valid_accession_ids, test_accession_ids


def main():
    args = parse_args()
    train_emb_tensor, valid_emb_tensor, test_emb_tensor, train_accession_ids, valid_accession_ids, test_accession_ids = get_protein_embedding(args.esm2_model, args.layer)

    distance_matrix = torch.cdist(train_emb_tensor, test_emb_tensor, p=2)
    hardness_protien = compute_task_hardness_from_distance_matrix (distance_matrix, aggr = 'mean_median')
    target_test_df = pd.read_csv(os.path.join(FS_MOL_DATASET_PATH, "targets", "test_proteins.csv"))
    chembl_ids = []
    ...
    print("correlation between protein hardness and delta_auprc: ", df['protein_hardness_mean'].corr(df['delta_auprc']))
    print("correlation between protein hardness and roc_auc: ", df['protein_hardness_mean'].corr(df['roc_auc']))
